...
impl TerminalRegistry {
    /// Spawns a local shell with `KUBECONFIG` scoped to the active cluster and returns the session id.
    /// Fails when the kubeconfig cannot be materialized or the worker thread cannot start.
    pub async fn start(
        self: Arc<Self>,
        ...
        let worker = thread::Builder::new()
            ...
            .spawn(move || {
                run_terminal_session(TerminalSessionConfig {
    ...
    /// Sends input bytes to the session; an error means the session is gone.
    ...
    /// Resizes the PTY with clamped dimensions.
    ...
    /// Requests session shutdown and returns `false` for an unknown session.
    ...
    /// Stops every session and waits up to three seconds for workers to finish.
...
fn run_terminal_session(config: TerminalSessionConfig) {
        ...
        while let Ok(event) = worker_rx.try_recv() {
            match event {
                ...
                WorkerEvent::ReaderError(error) => {
                    ...
                    reason = error;
                    request_stop(&mut killer, &mut stopped, &mut stop_deadline);
                }
...
pub(super) fn write_temp_kubeconfig(contents: &str) -> Result<TempPath, String> {
...
#[cfg(unix)]
pub(super) fn local_shell_candidates() -> Vec<LocalShell> {
