...
async fn list_pod_metrics(
    ...
    metrics::list_pod_metrics(&registry, request).await
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        // A second launch (Start menu, taskbar, or a second `KubeHive.exe`)
        // must focus the existing instance instead of spawning another one.
