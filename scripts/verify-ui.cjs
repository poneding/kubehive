...
(async () => {
  ...
  const logModeControls = await page.locator(".session-action-bar").evaluate((bar) => {
    ...
    return {
      ...
      timestampsDefaultOff: [...bar.querySelectorAll(".session-checkbox")].some((label) => label.textContent.trim() === "Timestamps" && !label.querySelector("input").checked),
  ...
  // Wrap reconfigures the live editor: the document must survive the toggle and
  // the content box must stop stretching past the scroller.
  const editorWrapCheckbox = page.locator(".session-secondary-actions .session-checkbox").filter({ hasText: "Wrap" });
  const measureEditorWrap = (content) => ({
