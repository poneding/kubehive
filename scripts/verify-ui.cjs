...
(async () => {
  ...
  const terminalThemeScrollbar = await page.locator(".container-terminal .xterm-viewport").evaluate((viewport) => { viewport.classList.add("is-scrolling"); const terminal = viewport.closest(".container-terminal"); const thumb = getComputedStyle(terminal).getPropertyValue("--terminal-scrollbar-thumb").trim(); const track = getComputedStyle(terminal).getPropertyValue("--terminal-scrollbar-track").trim(); const color = getComputedStyle(viewport).scrollbarColor; return Boolean(thumb && track && color !== "auto" && color !== ""); });
  ...
  const logModeControls = await page.locator(".session-action-bar").evaluate((bar) => {
    ...
    return {
      ...
      followLogs: bar.querySelectorAll('input[type="checkbox"]').length === 3,
