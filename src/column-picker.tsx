...
import { Button, Checkbox, Popover, PopoverContent, PopoverTrigger, ScrollArea, Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui";
...
export function ColumnPicker({ resource, language, defs, isVisible, onToggle, onReset }: {
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    <div className={cn("column-picker", open && "open")} onClick={(event) => event.stopPropagation()}>
      <Tooltip>
        ...
        <TooltipContent>{t(language, "columns")}</TooltipContent>
      </Tooltip>
      <PopoverContent
        align="end"
        sideOffset={8}
        collisionPadding={8}
        className="column-picker-popover w-[220px] p-[6px]"
        role="menu"
        aria-label={t(language, "columns")}
      ...
      >
        ...
        <ScrollArea className="column-picker-list" viewportClassName="column-picker-list-viewport">
          <div className="column-picker-list-content">
          {defs.map((column) => {
            ...
            return <label key={column.id} className={cn(column.required && "required")}>
              <Checkbox
                ...
                onCheckedChange={(nextChecked) => onToggle(column.id, nextChecked === true)}
              />
