...
export function Button({ className, variant = "default", size = "default", ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "default" | "secondary" | "ghost" | "outline" | "danger"; size?: "default" | "icon" | "sm" }) {
...
export function Progress({ value, tone = "green" }: { value: number; tone?: "green" | "amber" | "red" }) {
  return <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/7"><div className={cn("h-full rounded-full", tone === "green" && "bg-emerald-400", tone === "amber" && "bg-amber-400", tone === "red" && "bg-red-400")} style={{ width: `${Math.min(100, value)}%` }} /></div>;
}
