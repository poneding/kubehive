...
function BottomActionSheet({ clusterId, sessions, activeId, collapsed, language, appTheme, terminalTheme, terminalFont, terminalFontSize, terminalRuntimes, sessionCaches, onUpdateTerminalRuntimes, onUpdateSessionCaches, onActivate, onCloseSession, onCloseOthers, onCloseAll, onCreateSession, onToggleCollapsed, onApplied, onToast }: {
  ...
  const sessionTitle = (session: BottomSession) => `${session.mode === "terminal" ? (session.terminalTarget === "container" || (session.terminalTarget === undefined && session.item) ? "Container terminal" : "Local terminal") : session.mode === "logs" ? "Logs" : session.mode === "files" ? "Files" : session.mode === "edit" ? session.readOnlyReason ? "View" : "Edit" : "Create"} · ${session.label ?? session.item?.label ?? "cluster"}`;
  const terminalOption = language === "en" ? "New local terminal" : language === "zh-TW" ? "新增本機終端工作階段" : "新建本地终端会话";
  ...
  const renderFileTargetSelectors = (inPanel = false) => <>
    ...
    {containerOptions.length > 1
      ...
      : <div className={cn("session-target-label", inPanel && "file-explorer-target-panel-label")} aria-label="Container"><Container size={12} aria-hidden="true" /><strong title={selectedContainer || targetError || undefined}>{selectedContainer || (targetsLoading ? "Resolving..." : "Unavailable")}</strong></div>}
  ...
  const fileSessionTargets = fileExplorer ? <div className="file-explorer-session-targets">
    ...
    <div className="file-explorer-session-targets-overflow">
      <button type="button" aria-haspopup="dialog" aria-label={showPodTarget ? "Show Pod and Container selectors" : "Show Container selector"} title="Pod and Container selectors"><Container size={14} /></button>
      <div className="file-explorer-session-targets-panel" role="group" aria-label="Pod and Container selectors">{renderFileTargetSelectors(true)}</div>
    </div>
  </div> : undefined;
    ...
    <div className={cn("workspace-pane", workspaceView === "clusters" && "home-mode")}>
      {workspaceView === "clusters" ? <ClusterHome clusters={availableClusters} language={language} busyClusterId={clusterOperationId} onConnect={(target) => void connectAndOpenCluster(target)} onCloseConnection={(target) => void closeClusterConnection(target)} onSettings={(target) => setClusterSettingsId(target.id)} onRemove={removeCluster} onAdd={() => setAddClusterOpen(true)} /> : clusterConnection?.clusterId === activeCluster.id ? <ClusterConnectionPage cluster={activeCluster} language={language} state={clusterConnection} busy={clusterOperationId === activeCluster.id} onReconnect={retryClusterConnection} onClose={() => void closeClusterConnection(activeCluster)} /> : <>
        ...
        <main className="main-area">
          ...
          {bottomSessions.length > 0 && <BottomActionSheet clusterId={activeCluster.id} sessions={bottomSessions} activeId={activeBottomId} collapsed={bottomCollapsed} language={language} appTheme={resolvedTheme} terminalTheme={terminalAppearance} terminalFont={preferences.terminalFont} terminalFontSize={preferences.terminalFontSize} terminalRuntimes={terminalRuntimes} sessionCaches={bottomSessionCaches} onUpdateTerminalRuntimes={updateTerminalRuntimes} onUpdateSessionCaches={updateBottomSessionCaches} onActivate={(id) => { setActiveBottomId(id); setBottomCollapsed(false); }} onCloseSession={closeBottomSession} onCloseOthers={closeOtherSessions} onCloseAll={closeAllSessions} onCreateSession={openBottomSession} onToggleCollapsed={() => setBottomCollapsed((value) => !value)} onApplied={() => setDataRevision((value) => value + 1)} onToast={showToast} />}
        </main>
      </>}
    </div>
