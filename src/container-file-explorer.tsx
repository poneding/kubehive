...
type OperationDialog = { operation: FileOperation; entry?: ContainerFileEntry };
    ...
    : state.operation === "create-directory" ? "Create folder"
      : state.operation === "rename" ? "Rename"
        : state.operation === "move" ? "Move to path"
          : "Copy to path";
  const defaultValue = create ? "" : state.operation === "rename" ? state.entry?.name ?? "" : state.entry?.path ?? "";
...
export function ContainerFileExplorer({ target, appTheme, terminalFont, terminalFontSize, onToast }: {
  ...
  const onKeyboard = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    else if (event.key === "Delete" || event.key === "Backspace" && (event.metaKey || event.ctrlKey)) { event.preventDefault(); void remove(selected); }
  };
  ...
  return <div className={cn("container-file-explorer", `file-theme-${appTheme}`, dragging && "is-dragging")} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragging(false); }} onDrop={(event: DragEvent<HTMLDivElement>) => { event.preventDefault(); setDragging(false); void uploadFiles(event.dataTransfer.files); }}>
    ...
    {editor ? <div className="file-text-editor">
      ...
      {!loading && filtered.length > 0 && view === "list" && <table className="file-list"><thead><tr><th>Name</th><th>Size</th><th>Modified</th><th>Mode</th><th aria-label="Actions" /></tr></thead><tbody>{filtered.map((entry) => <tr key={entry.path} className={cn(entry.path === selectedPath && "selected")} onClick={() => setSelectedPath(entry.path)} onDoubleClick={() => void openEntry(entry)} onContextMenu={(event) => openEntryMenu(event, entry)}><td><span className={cn("file-entry-icon", `kind-${entry.kind}`)}>{fileIcon(entry, 15)}</span><div><strong>{entry.name}</strong>{entry.kind === "symlink" && <small>Symbolic link</small>}</div></td><td>{entry.kind === "directory" ? "—" : formatBytes(entry.size)}</td><td>{formatModified(entry.modifiedAt)}</td><td><code>{entry.permissions}</code></td><td><button aria-label={`Actions for ${entry.name}`} onClick={(event) => { event.stopPropagation(); openEntryMenu(event, entry); }}><MoreHorizontal size={14} /></button></td></tr>)}</tbody></table>}
      {!loading && filtered.length > 0 && view === "grid" && <div className="file-grid">{filtered.map((entry) => <button key={entry.path} className={cn("file-grid-item", entry.path === selectedPath && "selected")} onClick={() => setSelectedPath(entry.path)} onDoubleClick={() => void openEntry(entry)} onContextMenu={(event) => openEntryMenu(event, entry)}><span className={cn("file-entry-icon", `kind-${entry.kind}`)}>{fileIcon(entry, 27)}</span><strong>{entry.name}</strong><small>{entry.kind === "directory" ? "Folder" : formatBytes(entry.size)}</small></button>)}</div>}
    </div>}
    {!editor && <div className="file-explorer-status"><span>{filtered.length} item{filtered.length === 1 ? "" : "s"}{query ? ` matching · ${entries.length} total` : ""}</span>{selected ? <><strong>{selected.name}</strong><span>{selected.kind === "directory" ? "Folder" : formatBytes(selected.size)} · {selected.readable ? "read" : "no read"}/{selected.writable ? "write" : "no write"}</span></> : <span>Double-click to open · F2 rename · Delete remove</span>}</div>}
