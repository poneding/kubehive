...
export function ContainerFileExplorer({ target, appTheme, terminalFont, terminalFontSize, onToast }: {
  ...
  const toggleSelection = (entry: ContainerFileEntry, event?: { metaKey?: boolean; ctrlKey?: boolean; shiftKey?: boolean }) => {
    setSelectedPaths((current) => {
      const additive = Boolean(event?.metaKey || event?.ctrlKey || event?.shiftKey);
      if (!additive) return [entry.path];
      return current.includes(entry.path) ? current.filter((path) => path !== entry.path) : [...current, entry.path];
    });
  };
  ...
  return <div className={cn("container-file-explorer", `file-theme-${appTheme}`, dragging && "is-dragging")} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragging(false); }} onDrop={(event: DragEvent<HTMLDivElement>) => { event.preventDefault(); setDragging(false); void uploadFiles(event.dataTransfer.files); }}>
    ...
    {editor ? <div className="file-text-editor">
      ...
      {!loading && filtered.length > 0 && view === "list" && <table className="file-list"><thead><tr><th className="file-selection-column"><input type="checkbox" aria-label="Select all files" checked={filtered.length > 0 && filtered.every((entry) => selectedPaths.includes(entry.path))} onChange={(event) => setSelectedPaths(event.target.checked ? filtered.map((entry) => entry.path) : [])} /></th><th>Name</th><th>Size</th><th>Modified</th><th>Mode</th><th aria-label="Actions" /></tr></thead><tbody>{filtered.map((entry) => <tr key={entry.path} className={cn(selectedPaths.includes(entry.path) && "selected")} onClick={(event) => toggleSelection(entry, event)} onDoubleClick={() => void openEntry(entry)} onContextMenu={(event) => openEntryMenu(event, entry)}><td className="file-selection-column"><input type="checkbox" aria-label={`Select ${entry.name}`} checked={selectedPaths.includes(entry.path)} onClick={(event) => event.stopPropagation()} onChange={(event) => toggleSelection(entry, { ctrlKey: true, metaKey: true, shiftKey: event.nativeEvent.shiftKey })} /></td><td><span className={cn("file-entry-icon", `kind-${entry.kind}`)}>{fileIcon(entry, 15)}</span><div><strong>{entry.name}</strong>{entry.kind === "symlink" && <small>Symbolic link</small>}</div></td><td>{entry.kind === "directory" ? "—" : formatBytes(entry.size)}</td><td>{formatModified(entry.modifiedAt)}</td><td><code>{entry.permissions}</code></td><td><button aria-label={`Actions for ${entry.name}`} onClick={(event) => { event.stopPropagation(); openEntryMenu(event, entry); }}><MoreHorizontal size={14} /></button></td></tr>)}</tbody></table>}
      {!loading && filtered.length > 0 && view === "grid" && <div className="file-grid">{filtered.map((entry) => <button key={entry.path} className={cn("file-grid-item", selectedPaths.includes(entry.path) && "selected")} onClick={(event) => toggleSelection(entry, event)} onDoubleClick={() => void openEntry(entry)} onContextMenu={(event) => openEntryMenu(event, entry)}><span className="file-grid-check" aria-hidden="true">{selectedPaths.includes(entry.path) ? "✓" : ""}</span><span className={cn("file-entry-icon", `kind-${entry.kind}`)}>{fileIcon(entry, 27)}</span><strong>{entry.name}</strong><small>{entry.kind === "directory" ? "Folder" : formatBytes(entry.size)}</small></button>)}</div>}
    </div>}
