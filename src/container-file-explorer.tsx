import {
  ArrowLeft, ArrowUp, ClipboardCopy, Copy, Download, File, FileCode2, FilePlus2,
...
export function ContainerFileExplorer({ target, targetLoading = false, targetUnavailableTitle, targetUnavailableMessage, initialSnapshot, onSnapshotChange, appTheme, monoFont, contentFontSize, language, sessionTargetControls, onToast }: {
  ...
  const copyEntryPath = (entry: ContainerFileEntry) => {
    if (!navigator.clipboard?.writeText) { onToast("error", tr(language, "unableToCopyPath")); return; }
    void navigator.clipboard.writeText(entry.path)
      .then(() => onToast("success", tr(language, "pathCopied", { path: entry.path })))
      .catch(() => onToast("error", tr(language, "unableToCopyPath")));
  };
  ...
  const openEntryMenu = (event: ReactMouseEvent, entry: ContainerFileEntry) => {
    ...
    openContextMenu(event, [
      ...
      { type: "item", id: "copy-path", label: tr(language, "copyPath"), icon: ClipboardCopy, onSelect: () => copyEntryPath(entry) },
      { type: "separator" },
