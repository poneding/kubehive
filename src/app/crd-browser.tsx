...
import { renderResourceCell, resourceCellText } from "./resource-cells";
...
function CustomResourceList({ clusterId, discovered, namespaces, revision, crdName, query, onQueryChange, selectedNamespaces, setSelectedNamespaces, language, onBack, onInstance, onCreate, onRowAction, onOpenLink, onCopy }: {
  ...
  const columns = useMemo<Array<VirtualTableColumn<ResourceRow>>>(() => [
    ...visible.map((column) => ({ id: column.id, label: column.label, badgeLabel: column.id === "status" ? (item: ResourceRow) => resourceCellText("status", item) : undefined, render: (item: ResourceRow) => renderResourceCell(column.id, item, onOpenLink, language, onCopy) })),
