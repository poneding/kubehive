...
- 排障：Pod/工作负载日志、容器 exec 命令，以及 Pod / Service 端口的本地 TCP port-forward。详情面板通过 Port / Protocol / Address / Actions 表格逐端口提供转发、点击访问、复制地址和停止操作；每个目标端口最多保留一个转发。可自动分配端口、选择 `localhost` / `0.0.0.0`、指定 HTTP/HTTPS 打开方式，并在成功后用默认浏览器打开。转发定义会安全保存在应用配置目录，集群重新连接后自动恢复。Service 会按 EndpointSlice（兼容 Endpoints 回退）选择一个 ready Pod，再通过 kube-rs WebSocket 建立转发。
